...
function downAlert(userName, landing) {
  const content = `
    ...
    <p>La landing <strong>${landing.nombre}</strong> (${landing.marca}) está reportando como <strong style="color:#dc2626">CAÍDA</strong>.</p>
    <table style="width:100%;border-collapse:collapse;margin:16px 0;background:#fef2f2;border-radius:8px">
      <tr><td style="padding:12px 16px;font-size:14px">
        <strong>URL:</strong> <a href="${landing.url}" style="color:#6366f1">${landing.url}</a><br>
  ...
  `
  return baseTemplate(`⚠️ ${landing.nombre} está CAÍDA`, content)
}

function sslAlert(userName, landing, daysRemaining) {
  const content = `
    <p>Hola <strong>${userName}</strong>,</p>
    <p>El certificado SSL de <strong>${landing.nombre}</strong> vence en <strong style="color:#f59e0b">${daysRemaining} días</strong>.</p>
    ...
    <a href="${process.env.APP_URL || 'http://localhost:5173'}" style="display:inline-block;padding:12px 24px;background:#6366f1;color:#fff;text-decoration:none;border-radius:6px;font-size:14px">Ir al Dashboard</a>
  `
  return baseTemplate(`🔒 SSL próximo a vencer — ${landing.nombre}`, content)
}

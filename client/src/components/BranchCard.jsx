  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className={`relative z-10 p-5 flex flex-col h-full min-h-[280px] ${hasImage ? "text-white" : ""}`}>
          ...
          <div className="flex items-center gap-2 mb-3 flex-wrap">
            ...
            {templateType && (
              <span className={`text-xs px-2 py-0.5 rounded-full border ${hasImage ? "bg-white/10 backdrop-blur-sm border-white/20" : "bg-secondary/50 text-secondary-foreground border-border"}`}>
                {templateType.name}
              </span>
            )}
            {uptimeBadge}
          ...
          {visibleTemplateFields.map((f) => {
            ...
            if (f.fieldType === "password") return (
              <div key={f.name} className="flex items-center gap-1 text-xs mb-1">
          ...
          {/* Check & Logs row - only show if has URL */}
          {landing.url && (
            <div className="flex items-center gap-2 mb-2">
              <Button
                variant="outline"
          ...
          <div className={`text-xs mt-auto ${hasImage ? "opacity-70" : "text-muted-foreground"}`}>
            ...
            {templateType && <span className="ml-2 opacity-60">{templateType.icon} {templateType.name}</span>}
          </div>
        </div>
      </div>
    </motion.div>

...
export default function CategoryManager() {
  ...
  const handleCreate = async (e) => {
    ...
    await createCat.mutateAsync({ name: newName, color: newColor })
    setNewName("")
  ...
  const handleUpdate = async (id) => {
    ...
    await updateCat.mutateAsync({ id, data: { name: editing.name, color: editing.color } })
    setEditing(null)
  }
              ...
              <>
                ...
                {isAdmin && (
                  <div className="flex gap-1">
                    ...
                    <button onClick={() => { if (confirm(`¿Eliminar "${cat.name}"?`)) deleteCat.mutateAsync(cat.id) }} className="p-1 rounded hover:bg-accent text-destructive"><Trash2 className="w-4 h-4" /></button>
                  </div>
                )}
              </>
            )}
